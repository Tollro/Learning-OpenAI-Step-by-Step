...
if __name__ == "__main__":
    ...
    for start_idx in range(0, total, BATCH_SIZE):
        ...
        # 为当前批次生成文本和ID
        instructions = [embed_text_from_item(item) for item in batch_data]
        documents = [document_from_item(item) for item in batch_data]
        ids = [str(start_idx + i) for i in range(len(batch_data))]

        # 添加当前批次
        treatment_db.add_data("treatments", ids, instructions, documents)
        print(f"已添加 {end_idx}/{total} 条数据")
